...
def template_classifier(l_rate = 0.01):
    ...
    extra_dense = Dense(len_feature , activation = 'relu', input_shape = (len_feature,))(pooled_template)
    softmax_proba = classifier()(extra_dense)     
...
def train_with_generator(snap_weight = None):
    ...
    prefix = 'extradense-sgdlr'+str(sgd_lr)+'-featperbatch'+str(feat_per_batch)
    model, mean_model, _, _, _ = template_classifier(l_rate = sgd_lr)
    model.summary()
    if snap_weight is not None:
        assert os.path.isfile(snap_weight)
        print 'loading weight_predictor weights from ',snap_weight
